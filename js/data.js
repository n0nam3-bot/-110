...
export async function getESPNGames(sportKey) {
  const s = CONFIG.sports[sportKey];
  if (!s) return [];
  const ck = `espn_${sportKey}`;
  const cached = memGet(ck);
  if (cached) return cached;

  const url  = CONFIG.espn.scoreboard(s.espnSport, s.espnLeague);
  ...
  memSet(ck, games, CONFIG.cache.espn);
  return games;
}
...
export async function getSleeperInjuries() {
  ...
  if (cached) return cached;
  if (!_sleeperPlayers) {
    _sleeperPlayers = await fetchJSON(CONFIG.sleeper.players);
  }
  if (!_sleeperPlayers) return {};
  const injured = {};
  for (const [id, p] of Object.entries(_sleeperPlayers)) {
...
// ─── TheSportsDB (team logos fallback) ──────────────────────────────────────

export async function getTeamInfo(teamName) {
  const ck = `tsdb_${teamName}`;
  const cached = memGet(ck);
  if (cached) return cached;
...
export function mergeOddsIntoGames(games, oddsData) {
  return games.map(g => {
    const match = oddsData.find(o =>
      o.homeTeam.toLowerCase().includes(g.homeTeam.abbr?.toLowerCase() || "") ||
      o.homeTeam.toLowerCase().includes(g.homeTeam.name?.split(" ").pop()?.toLowerCase() || "") ||
      o.awayTeam.toLowerCase().includes(g.awayTeam.abbr?.toLowerCase() || "")
    );
    if (!match) return g;

    // Consensus odds (average across books)
    ...
    return {
      ...
      odds: {
        spread: spread ? {
          home: spread.outcomes?.find(o => o.name === match.homeTeam),
          away: spread.outcomes?.find(o => o.name === match.awayTeam)
        } : null,
        total: total ? {
          over:  total.outcomes?.find(o => o.name === "Over"),
          under: total.outcomes?.find(o => o.name === "Under")
        } : null,
        moneyline: h2h ? {
          home: h2h.outcomes?.find(o => o.name === match.homeTeam),
          away: h2h.outcomes?.find(o => o.name === match.awayTeam)
        } : null
      }
    };
  });
